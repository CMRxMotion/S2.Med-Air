...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        coords = torch.stack(torch.meshgrid([coords_h, coords_w, coords_d]))  # 3, Wh, Ww Dd
...
class SwinTransformerBlock(nn.Module):
    ...
    def flops(self):
        ...
        # W-MSA/SW-MSA
        nW = H * W * D / self.window_size[0] / self.window_size[1] / self.window_size[2]
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        x0 = x[:, 0::2, 0::2, :, :]  # B H/2 W/2 D C
        x1 = x[:, 1::2, 0::2, :, :]  # B H/2 W/2 D C
        x2 = x[:, 0::2, 1::2, :, :]  # B H/2 W/2 D C
        x3 = x[:, 1::2, 1::2, :, :]  # B H/2 W/2 D C
        x = torch.cat([x0, x1, x2, x3], -1)  # B H/2 W/2 4*C
        x = x.view(B, -1, 4 * C)  # B H/2*W/2*D 4*C
...
class PatchExpand(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(B, H, W, D, C)
        x = rearrange(x, 'b h w d (p1 p2 p3 c)-> b (h p1) (w p2) (d p3) c', p1=2, p2=2, p3=1, c=C // 4)
        x = x.view(B, -1, C // 4)
        x = self.norm(x)
...
class FinalPatchExpand_X4(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=4, norm_layer=nn.LayerNorm):
        ...
        self.expand = nn.Linear(dim, 192 * dim, bias=False)
        self.output_dim = dim
    ...
    def forward(self, x):
        ...
        x = x.view(B, H, W, D, C)
        x = rearrange(x, 'b h w d (p1 p2 p3 c)-> b (h p1) (w p2) (d p3) c', p1=self.dim_scale[0], p2=self.dim_scale[1],
                      p3=self.dim_scale[2], c=C // (self.dim_scale[0] * self.dim_scale[1] * self.dim_scale[2]))
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 shift_size=[0, 0, 0] if (i % 2 == 0) else [i // 2 for i in window_size],
                                 mlp_ratio=mlp_ratio,
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=(448, 448, 12), patch_size=(8, 8, 3), in_chans=1, embed_dim=96, norm_layer=None):
        super().__init__()
    ...
    def flops(self):
        ...
        flops = Ho * Wo * Do * self.embed_dim * self.in_chans * (
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=[448, 448, 12], patch_size=[8, 8, 3], in_chans=1, num_classes=2,
                 embed_dim=48, depths=[2, 2, 2, 2], depths_decoder=[1, 2, 2, 2], num_heads=[3, 6, 12, 24],
                 window_size=[7, 7, 4], mlp_ratio=4., qkv_bias=True, qk_scale=None,
    ...
    def up_x4(self, x):
        ...
        B, L, C = x.shape
        assert L == H * W * D, "input features has wrong size"

        if self.final_upsample == "expand_first":
            x = self.up(x)
            x = x.view(B, 8 * H, 8 * W, D * 3, -1)
