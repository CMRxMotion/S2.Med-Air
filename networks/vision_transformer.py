...
class SwinUnet(nn.Module):
    def __init__(self, config, img_size=224, in_chans=3, num_classes=21843, zero_head=False, vis=False):
        ...
        self.swin_unet_encoder = SwinTransformerSys_en(img_size=img_size,
        ...
        self.swin_unet_decoder = SwinTransformerSys_de(img_size=img_size,
                                patch_size=config.MODEL.SWIN.PATCH_SIZE,
                                in_chans=in_chans,
                                num_classes=self.num_classes,
                                embed_dim=config.MODEL.SWIN.EMBED_DIM,
    ...
    def forward(self, x):
        x, x_sample = self.swin_unet_encoder(x)
        logits = self.swin_unet_decoder(x, x_sample)
        return logits
    ...
    def load_from_en(self, pretrained_path):
        if pretrained_path is not None:
            print("pretrained_path:{}".format(pretrained_path))
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
